...
interface PasteVoErrorModalProps {
  isOpen: boolean
  pasteText: string
...
export function PasteVoErrorModal({
  isOpen,
  pasteText,
  onChangePasteText,
  onClose,
  onApply,
}: PasteVoErrorModalProps) {
      ...
      <div className="bg-background border border-border rounded-xl shadow-2xl max-w-md w-full p-5 space-y-3">
        ...
        <div className="flex items-center justify-end gap-2 pt-2 border-t">
          ...
          <button
            type="button"
            onClick={onApply}
            className="px-4 py-1.5 text-xs bg-red-600 hover:bg-red-700 text-white font-medium rounded-md transition-colors cursor-pointer"
          >
